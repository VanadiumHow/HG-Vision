...
using VisionProgram.UserControls.UITabControl.TabControlEX;

/****************************************************************
*****************************************************************/
namespace VisionProgram.UserControls.UITabControl
{
    public partial class TabControlEx : TabControl
    {
        ...
        private void DrawTabPages(Graphics g)
        {
            ...
            if (alignHorizontal)
            {
                ...
                if (hasUpDown)
                {
                    if (Native.IsWindowVisible(upDownButtonHandle))
                    {
                        Native.RECT upDownButtonRect = new Native.RECT();
            ...
            for (int index = 0; index < base.TabCount; index++)
            {
                ...
                if (selected)
                {
                    baseColor = GetColor(_baseColor, 0, -90, -60, -28);//-45, -30, -14
                }
                else if (hover)
                {
        ...
        private class UpDownButtonNativeWindow : NativeWindow, IDisposable
        {
            ...
            private bool LeftKeyPressed()
            {
                ...
                {
                    return (Native.GetKeyState(Native.VK_RBUTTON) < 0);
                }
                else
                {
                    return (Native.GetKeyState(Native.VK_LBUTTON) < 0);
                }
            }
            ...
            private void DrawUpDownButton()
            {
                ...
                Native.GetClientRect(base.Handle, ref rect);

                Rectangle clipRect = Rectangle.FromLTRB(
                    rect.Top, rect.Left, rect.Right, rect.Bottom);

                Point cursorPoint = new Point();
                Native.GetCursorPos(ref cursorPoint);
                Native.GetWindowRect(base.Handle, ref rect);

                mouseOver = Native.PtInRect(ref rect, cursorPoint);
                    ...
                    case Native.WM_PAINT:
                        if (!_bPainting)
                        {
                            Native.PAINTSTRUCT ps =
                                new Native.PAINTSTRUCT();
                            _bPainting = true;
                            Native.BeginPaint(m.HWnd, ref ps);
                            DrawUpDownButton();
                            Native.EndPaint(m.HWnd, ref ps);
                            _bPainting = false;
                            m.Result = Native.TRUE;
                        }
                        else
                        {
                            base.WndProc(ref m);
