...
namespace Obj.Obj_File
{
    ...
    public class IniHelper
    {
        ...
        /// <summary>
        /// ini内容为中文时，防止读取乱码，需用此方法
        /// </summary>
        /// <param name="area"></param>
        /// <param name="key"></param>
        /// <param name="def"></param>
        /// <returns></returns>
        public string IniReadValue_UTF8(string area, string key, string def = "")
        {
            if (File.Exists(path))
            {
                byte[] bytes = new byte[1024];
                int i = GetPrivateProfileString(System.IO.Path.GetFileNameWithoutExtension(area), key, def, bytes, 1024, path); 			//读取INI文件
                return Encoding.UTF8.GetString(bytes, 0, i);//返回INI文件的内容
            }
            else
            {
                MessageBox.Show("未找到ini文件");
        ...
        {
            byte[] temp = new byte[2255];

            int i = GetPrivateProfileString(section, key, "", temp, 2255, this.path);
        ...
        /// 读取ini文件的所有段落名
        /// </summary>    
        public List<string> IniReadAllSection()
        {
            byte[] allSection = IniReadValues(null, null);
            string[] tmp = ByteToString(allSection);
            List<string> AllSection = new List<string>();
            for (int i = 0; i < tmp.Length; i++)
            {
                if (tmp[i] != "")
                {
                    AllSection.Add(tmp[i]);
                }
            }
            return AllSection;
        }
        ...
        /// 读取ini文件的某段落下所有键名
        /// </summary>    
        public List<string> IniReadAllKeys(string Section)
        {
            byte[] sectionByte = IniReadValues(Section, null);
            string[] tmp = ByteToString(sectionByte);
        ...
        public static void AddRoDatabase(string area, string code, string des, string filename)
        {
            ...
            {
                IniHelper.WritePrivateProfileString(area, code, des, str); 			//修改INI文件中用户节点的内容
            }
            else
            {
        ...
        public string inimath(string code, string filename)
        {
            ...
            if (File.Exists(str)) 											//判断是否存在该INI文件
            {
                des = IniReadValue(IniHelper.strOne, code, "");
                return des;
                //读取INI文件中用户节点的内容
            }
            else
